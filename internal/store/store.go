...
var logger = applog.Component("store")
...
func (s *Store) Update(ctx context.Context, sl *slot.Slot) error {
	...
	if err := s.rdb.Set(ctx, slotKey, data, s.ttl).Err(); err != nil {
		logger().Error().Str("error_code", "ERR_STORE_UPDATE").Str("slot_id", sl.ID).
...
func (s *Store) Delete(ctx context.Context, slotID string) error {
	...
	if _, err := pipe.Exec(ctx); err != nil {
		logger().Error().Str("error_code", "ERR_STORE_DELETE").Str("slot_id", slotID).
...
func (s *Store) GetByCode(ctx context.Context, code string) (*slot.Slot, error) {
	...
	if err != nil {
		...
		logger().Error().Str("error_code", "ERR_STORE_CODE_LOOKUP").Msg("code index lookup failed")
...
func (s *Store) get(ctx context.Context, key string) (*slot.Slot, error) {
	...
	if err != nil {
		...
		logger().Error().Str("error_code", "ERR_STORE_GET").Msg("Redis GET failed")
		return nil, fmt.Errorf("store.get: %w", err)
	}

	var sl slot.Slot
	if err := json.Unmarshal(data, &sl); err != nil {
		logger().Error().Str("error_code", "ERR_STORE_UNMARSHAL").Msg("slot JSON corrupt")
