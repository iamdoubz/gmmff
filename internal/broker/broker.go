...
import (
	...
	"github.com/gmmff/gmmff/internal/crypto"
	applog "github.com/gmmff/gmmff/internal/log"
	"github.com/gmmff/gmmff/internal/slot"
	"github.com/gmmff/gmmff/internal/store"
	"github.com/gmmff/gmmff/pkg/protocol"
...
func (b *Broker) Run(ctx context.Context) {
	...
	for {
		select {
		case <-ctx.Done():
			log.Info().Msg("hub stopping")
			return

		case msg := <-b.register:
			b.conns[msg.c.id] = msg.c
			log.Debug().Str("conn_id", msg.c.id).Msg("connection registered")
